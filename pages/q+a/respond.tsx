...
export function QuestionForm({ q, onPop }: QuestionFormProps) {
  ...
  const [response, setResponse] = useState<ComplexQuesiton>(() => ({
    d: new Date(q.d),
    c: [
      ['q', q.q],
      ['a', ''],
  ...
  async function $answer() {
    setLoading(true);
    const res = await fetch(`/api/q+a/answer-request`, {
      method: 'POST',
      body: JSON.stringify({ id: q._id, response: trimResponse(response) }),
  ...
  async function $deny() {
    setLoading(true);
    const res = await fetch(`/api/q+a/deny-request`, {
      method: 'POST',
        ...
        <div className={c.editor}>
          ...
          {response.c.map(([k, v], i) => {
            function $change(ev: React.ChangeEvent<HTMLTextAreaElement>) {
              setResponse((r) => ({
                ...r,
                c: [...r.c.slice(0, i), [k, ev.target.value], ...r.c.slice(i + 1)],
              }));
              setI((i) => i + 1);
            }
            ...
            function $moveUp() {
              setResponse((r) => ({
                ...r,
                c: [...r.c.slice(0, i - 1), [k, v], ...r.c.slice(i - 1, i), ...r.c.slice(i + 1)],
              }));
              setI((i) => i + 1);
            }
            ...
            function $moveDown() {
              setResponse((r) => ({
                ...r,
                c: [...r.c.slice(0, i), ...r.c.slice(i + 1, i + 2), [k, v], ...r.c.slice(i + 2)],
              }));
              setI((i) => i + 1);
            }
            ...
            function $swap() {
              setResponse((r) => ({
                ...r,
                c: [...r.c.slice(0, i), [k === 'q' ? 'a' : 'q', v], ...r.c.slice(i + 1)],
              }));
              setI((i) => i + 1);
            }
            ...
            return (
              <div key={i} style={{ display: 'flex' }}>
                <div>
                  <button onClick={$moveUp}>u</button> <button onClick={$moveDown}>d</button>{' '}
                  <button onClick={$delete}>-</button> <button onClick={$swap}>{k}</button>
                </div>
                :{' '}
                <textarea
                  onKeyDown={$keyDown}
          ...
          <div>
            <button
              onClick={() =>
                setResponse((r) => ({
                  ...r,
                  c: [...r.c.slice(0, i), ['a', ''], ...r.c.slice(i + 1)],
                }))
              }
            >
              +q
...
export default function QARespond() {
  ...
  const { data, revalidate } = useSWR<QAGetRequestsResponse>(
    session && '/api/q+a/get-requests',
    fetcher
  );
