...
function registerAllPacks() {
  ...
  while (sortedAndParsedPacks.length > 0) {
    ...
    if (loadAfter) {
      const x = sortedAndParsedPacks.find((pack) => loadAfter.listedIds.includes(pack.id));
      if (x) {
        notLoadYet = 'loadAfter:' + x.id;
      }
    }
    pack.parsed.filter((x) => x.type === 'import').forEach((imp) => {
      const x = sortedAndParsedPacks.find((pack) => pack.id === imp.packID);
      if (x) {
        notLoadYet = 'import:' + x.id;
      }
    });
    if (beforeRequirements[pack.id]) {
      const x = beforeRequirements[pack.id].find((d) => !loadedPackIds.includes(d));
      if (x) {
...
Promise.all([
  ...
  try {
    ...
    allSortedAndParsedPacks.forEach((pack) => {
      ...
      const title = titleNode ? titleNode.title : 'Unnamed Pack';
