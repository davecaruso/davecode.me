const targetModule = require('next/dist/server/on-demand-entry-handler')

const EventEmitter = require('events');
const { join, posix } = require('path');
const { parse } = require('url');
const Log = require('next/dist/build/output/log');
const { normalizePagePath, normalizePathSep } = require('next/dist/next-server/server/normalize-page-path');
const { pageNotFoundError } = require('next/dist/next-server/server/require');
const findPageFile = require('next/dist/server/lib/find-page-file');
const getRouteFromEntrypoint = require('next/dist/next-server/server/get-route-from-entrypoint').default;

const ADDED = targetModule.ADDED;
const BUILDING = targetModule.BUILDING;
const BUILT = targetModule.BUILT;
const entries = targetModule.entries;
...
function onDemandEntryHandler(watcher, multiCompiler, { pagesDir, pageExtensions, maxInactiveAge, pagesBufferLength, }) {
    ...
    return {
        async ensurePage(page) {
            ...
            let pagePath = await findPageFile.findPageFile(pagesDir, normalizedPagePath, pageExtensions);
            ...
            const bundleFile = pagePath === 'next/dist/pages/_error' ? '/' + pagePath : normalizePagePath(page);
            const serverBundlePath = posix.join('pages', bundleFile);
            const clientBundlePath = posix.join('pages', bundleFile);
            let absolutePagePath = pagePath.startsWith('next/dist/pages')
                ? require.resolve(pagePath)
                : join(pagesDir, pagePath);

            page = pagePath === 'next/dist/pages/_error' ? '/' + pagePath : posix.normalize(page);
...
targetModule.default = onDemandEntryHandler;
